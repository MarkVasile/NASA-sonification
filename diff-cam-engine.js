var DiffCamEngine = (function() {
  ...
  var videoContext;   // context 2d which we use to capture a video frame
	...
	function init(options) {
		...
		diffCanvas = options.diffCanvas || document.createElement('canvas');
		isReadyToDiff = false;
		...
		// prep capture canvas
    const nebula = document.querySelector('video')
    nebula.addEventListener('loadeddata', () => {
      captureWidth = nebula.videoWidth
      captureHeight = nebula.videoHeight
      ...
      console.log('Diff canvas dimensions:', diffWidth, diffHeight)
	...
	function start() {
    console.log('STARTING...', capture, captureIntervalTime)
	...
	function processDiff(diffImageData) {
      ...
      // Pixel differences can be reported differently for each R, G, B channel
			var pixelDiff = rgba[i] * 0.3 + rgba[i + 1] * 0.4 + rgba[i + 2] * 0.3;
